...
### 2.3 User Stories
- As a user, I can **sign in with Google** so I don’t need to manage API keys.
- As a user, I can use Quorum for free (20 uses/month) before deciding to pay.
- As a user, I can **upgrade to Pro ($10/mo)** for unlimited access to all models.
...
#### FR5 — Auth & Freemium
- Users sign in with Google OAuth (no passwords).
- JWT sessions (30-day) stored in `chrome.storage.local`.
- Free tier: 20 uses/month, limited to 3 models (one per provider).
- Pro tier ($10/mo): unlimited uses, all models.
- Tier limits enforced server-side; frontend reflects tier from `/auth/me` on every mount.
- Stripe Checkout for upgrades; tier updates automatically without sign-out.

#### FR6 — Provider Abstraction
- Provider/model is configured via adapter layer so additional providers can be added later.
- API keys are backend-managed (stored in `backend/.env`), never exposed to or supplied by users.
...
- **M4 — Polish + Multi-Provider** ✅
  - Mixed-model mode (agents from multiple providers)
  - Image/vision support
  - LaTeX rendering, side panel, session persistence
- **M5 — Freemium + Auth** ✅
  - Google Sign-In via OAuth
  - Supabase user/usage persistence
  - Free tier (20 uses/month, 3 models) enforced server-side
  - Stripe Checkout for Pro ($10/mo)
  - Automatic tier update after payment (no sign-out required)
  - Backend-managed API keys (users no longer supply their own)
